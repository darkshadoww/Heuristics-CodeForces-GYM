...
int main()
{
    ...
    /*    for(int k=1; k<=last; k++)
          ...
          for(int j=1; j<=i; j++)
          {
          dp[k][i][j] = max(dp[k][i-1][j], dp[k-v[i-1].F][i-1][j-1]+v[i-1].S);
          }
          }
          }
          */
                ...
                for(int i=1; i<=n; i++)
                {
                    for(int j=1; j<=cnt; j++)
                    {
                        ...
                        if(j<cnt2)
                            ...
                            dp[k][i][j] = max(dp[k][i][j],max(dp[k][i-1][j], dp[k-v[i-1].F.F][i-1][j-1]+v[i-1].F.S));
                    }
                }
            }
    ...
    last = save_last;
    int save_cnt = cnt;
    for(int ll=save_last; ll>=0 && cnt>0; ll--)
        if(net_access[ll])
        {
            last--;
            tmp[ll]=1;
    ...
    for(int k=1, checker=1; k<=last; k++,checker++)
    {
        ...
        for(int i=1; i<=n; i++)
        {
            for(int j=1; j<=cnt; j++)
            {
                ...
                {
                    if(dp[k][i][j] < dp[k-v[i-1].F.F][i-1][j-1]+v[i-1].F.S)
                    {
                        parent[k][i][j] = 2;
                        dp[k][i][j] = dp[k][i-1][j], dp[k-v[i-1].F.F][i-1][j-1]+v[i-1].F.S;
                    }
                    if(dp[k][i][j] < dp[k][i-1][j])
                        parent[k][i][j]=1;
                    dp[k][i][j] = max(dp[k][i][j],max(dp[k][i-1][j], dp[k-v[i-1].F.F][i-1][j-1]+v[i-1].F.S));
    ...
    printf("%d ", ans_mx);
    int basek=last, basei=n, basej=cnt;
    vector<int> anss;
    while(basej>0)
    {
